...
import com.example.cyc.ChatMsgViewAdapter;
import com.example.cyc.SoundMeter;
import com.example.darren.lostfinding.Chat_mainActivity;
...
public class ChatingActivity extends Activity implements OnClickListener {
    ...
    private ArrayList<ChatMsgEntity> mDataArrays = new ArrayList<ChatMsgEntity>();
    ...
    private Gdata app;
    private ArrayList<ChatMsgEntity> chatList;
    ...
    final Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 1) {
                ...
                if(CL!=null){
                    mDataArrays.add(CL.get(0));
                    app.getChatData().clear();
    ...
    public void initView() {
        ...
        if(socket==null){
            try {
                socket = new WebSocket(new URI(app.ws_add + srcID));
                socket.setAPP(app);
        ...
        */
        ...
        Cursor cur=app.getdb().selectAllMSG(dstID);
        for(cur.moveToFirst();!cur.isAfterLast();cur.moveToNext())
        {
            ...
            int bColumn = cur.getColumnIndex("iscome");
            chatList.add(new ChatMsgEntity(cur.getString(nameColumn),cur.getString(timeColumn),cur.getString(textColumn),cur.getString(bColumn).contains("1")?true:false));
        }
        mDataArrays=(ArrayList)chatList.clone();
        chatList = app.getChatData().get(dstID);
        if (chatList != null && !chatList.isEmpty()) {
            app.getChatData().get(dstID).clear();
        }
        ...
        mAdapter = new ChatMsgViewAdapter(this, mDataArrays,srcID);
        mAdapter.setapp(app);
    ...
    private void send() {
        ...
        if (contString.length() > 0) {
            ...
            entity.setName(dstID);
            entity.setMsgType(false);
            entity.setText(contString);
            app.getdb().insert(entity.getName(),entity.getDate(),entity.getText(),entity.getMsgType()?"1":"0");
